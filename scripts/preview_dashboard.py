...
GAME_EXE = "Palworld-Win64-Shipping.exe"
...
def seed_status(unsafe: bool):
    ...
    The violation case is generated by calling the real
    audio_safety.check_audio_settings() (the same function daemon.py's
    _check_audio_safety calls) against a settings dict shaped like OBS's
    GetInputSettings response, rather than hardcoding a violation message
    here - so the preview can never drift from what the guard actually
    says (see src/audio_safety.py)."""
    if unsafe:
        audio_exes = [GAME_EXE, "discord.exe"]
    else:
        audio_exes = [GAME_EXE]

    settings = {"executable_list": [{"value": exe} for exe in audio_exes]}
    # Mirrors daemon.py's _audio_allowed_exes under exclusive_mode (the
    # default): only the currently streaming game is allowed, nothing else.
    allowed_exes = {GAME_EXE}
    violations = audio_safety.check_audio_settings(settings, allowed_exes)
    audio_violations = [v.message for v in violations]
    audio_ok = not any(v.severity == "stop" for v in violations)
    status = "OK" if audio_ok else "ISSUE"
